...
def download_iclr21(client, outdir='./', get_pdfs=False):
    '''
    ...
    id_to_submission = {
        note.id: note for note in openreview.tools.iterget_notes(client, invitation = 'ICLR.cc/2021/Conference/-/Blind_Submission')
    }
    ...
    reviews = openreview.tools.iterget_notes(
        client, invitation='ICLR.cc/2021/Conference/Paper.*/-/Official_Review')
    ...
    for cat in rl_submissions.keys():
        ...
        with open(os.path.join(outdir, 'iclr21_rl_{}.json'.format(cat)), 'w') as file_handle:
            for forum_metadata in rl_submissions[cat]:
                file_handle.write(json.dumps(forum_metadata) + '\n')
...
if __name__ == '__main__':
    ...
    download_iclr21(client, outdir, get_pdfs=args.get_pdfs)
